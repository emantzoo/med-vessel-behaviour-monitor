...
**What it shows:** One row per vessel, sorted by compounded risk score (highest first). The single most important table in the app.
...
**Interactions:** Click a row to filter the map to that vessel's events and pre-select it for the Vessel Investigation subtab. Use the slider to control how many vessels appear.
...
**Interactions:** Select a vessel from the dropdown, or click a row in Vessel Summary / a marker on the map. The dropdown is pre-populated with the highest-risk vessel.
