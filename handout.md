...
- Intentional disabling flag: GFW `gap_intentional_disabling = True` = 1.5× (GFW's own classification of deliberate AIS switch-off)
- Implied speed during gap: `gap_implied_speed_knots > 8 kn` = 1.4× (fast transit while dark — evasion pattern), `> 4 kn` = 1.2× (vessel was underway, not anchored)
...
- **`unregulated_flag_in_gfcm_area`** — vessel flagged to a non-GFCM contracting party or EU member state (medium severity). Mutually exclusive with stateless_vessel. Only fires for fishing vessels (vessel_class/vessel_type); non-fishing vessels with non-GFCM flags are exempt. ICCAT-authorized distant-water CPCs (JPN, KOR, CHN, TWN, ISL, NOR, GBR, and small-quota holders) are included in the authorized set and do not fire this leaf.
