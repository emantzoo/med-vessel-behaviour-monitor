...
The **Fishing Activity subtab** renders a three-layer PyDeck map: (1) FDI effort rectangles (0.5° c-square PolygonLayer, blue scale: light blue <50d → medium blue 50-500d → dark blue 500-2000d → navy >2000d), (2) grey background dots of fishing events inside MPAs (>=0.5h, capped at 2,000), and (3) coloured foreground dots for events firing risk tree leaves — colour encodes severity (red = closed area, orange = low-effort/no auth/unregulated, blue = general MPA), larger dots = higher severity. Clicking a foreground marker highlights the corresponding vessel in the table below the map.
